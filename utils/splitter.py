...
def split_train_val_test_idx(idx, smiles_list, frac_train=0.8, frac_valid=0.1, frac_test=0.1, sort=False, seed=42):
    random.seed(seed)
    ...
    # 根据索引获取对应的 SMILES 文本列表
    train_smiles = [smiles_list[i] for i in train_idx]
    valid_smiles = [smiles_list[i] for i in valid_idx]
    test_smiles = [smiles_list[i] for i in test_idx]

    return train_idx, valid_idx, test_idx, train_smiles, valid_smiles, test_smiles


def split_train_val_test_idx_stratified(idx, smiles_list, y, frac_train=0.8, frac_valid=0.1, frac_test=0.1, sort=False, seed=42):
    random.seed(seed)
...
def scaffold_split_train_val_test(index, smiles_list, frac_train=0.8, frac_valid=0.1, frac_test=0.1, sort=False):
    ...
    # 根据索引获取对应的 SMILES 文本列表
    train_smiles = [smiles_list[i] for i in train_index]
    valid_smiles = [smiles_list[i] for i in val_index]
    test_smiles = [smiles_list[i] for i in test_index]

    return train_index, val_index, test_index, train_smiles, valid_smiles, test_smiles
...
def random_scaffold_split_train_val_test(index, smiles_list, frac_train=0.8, frac_valid=0.1, frac_test=0.1,
    ...
    # 获取与每个索引相关联的 SMILES 字符串列表
    train_smiles = [smiles_list[idx] for idx in train_idx]
    val_smiles = [smiles_list[idx] for idx in valid_idx]
    test_smiles = [smiles_list[idx] for idx in test_idx]

    return train_index, val_index, test_index, train_smiles, val_smiles, test_smiles
...
def scaffold_split_balanced_train_val_test(index, smiles_list,
    ...
    # 获取与每个索引相关联的 SMILES 字符串列表
    train_smiles = [smiles_list[idx] for idx in train_idx]
    val_smiles = [smiles_list[idx] for idx in val_idx]
    test_smiles = [smiles_list[idx] for idx in test_idx]

    return train_idx, val_idx, test_idx, train_smiles, val_smiles, test_smiles
