    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0:
            ...
            for i in range(self.num_experts):
                ...
                cur_res=cur_res.unsqueeze(2).unsqueeze(3)
                lora_res = cur_res.permute(0, 3, 1, 2).contiguous()
            ...
            lora_res = lora_res.unsqueeze(2).unsqueeze(2)
            lora_res = lora_res + temp_lora_res

            lora_res = lora_res.view(B, L)
...
class MoEGate(nn.Module):
    def __init__(self, d, M=4, K=1, noisy_gating=False):
...
class SparseDispatcher(object):
    ...
    def __init__(self, num_experts, gates):
        ...
        if dim > 2:
            # 去除多余的维度
            gates = gates.squeeze(0)  # [batch_size, embed_dim]
